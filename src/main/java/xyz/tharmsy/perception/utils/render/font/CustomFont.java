...
public class CustomFont {
    ...
    protected CharacterData[] charData = new CharacterData[256];
    protected Font font;
    ...
    public void drawChar(CharacterData[] chars, char c, float x, float y) throws ArrayIndexOutOfBoundsException {
        try {
    ...
    public int getStringWidth(String text) {
        ...
        for (char c : text.toCharArray()) {
            if (c < charData.length) {
                width += charData[c].width - 8 + this.charOffset;
            }
        }
        return width / 2;
    }
